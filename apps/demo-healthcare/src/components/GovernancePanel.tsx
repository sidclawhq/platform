...
export function GovernancePanel({ agentId, apiKey }: GovernancePanelProps) {
  ...
  const handleApprovalAction = async (approvalId: string, action: 'approve' | 'deny', note: string, traceId?: string) => {
    await fetch('/api/approval-action', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ approvalId, action, apiKey, note, traceId, agentId }),
    });
    await poll();
  };

  const handleApprove = async (approvalId: string, note: string, traceId?: string) => {
    await handleApprovalAction(approvalId, 'approve', note, traceId);
  };
  ...
  return (
    <div className="flex w-1/2 flex-col">
      ...
      <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
        {traces.length === 0 && pendingApprovals.length === 0 && (
          <div className="mt-8 text-center text-sm text-[#71717A]">
            <p>Governance events will appear here as the clinical AI takes actions.</p>
            <p className="mt-2">Click a recommendation on the left to see governance in action.</p>
          </div>
        )}
        ...
        {pendingApprovals.map((approval) => (
          <ApprovalCard
            key={approval.id}
            approval={approval}
            onApprove={(note) => handleApprove(approval.id, note, approval.trace_id)}
      ...
      <div className="border-t border-[#2A2A2E] px-6 py-3 flex items-center justify-between">
        <span className="text-xs text-[#71717A]">MedAssist Health — Demo Environment</span>
