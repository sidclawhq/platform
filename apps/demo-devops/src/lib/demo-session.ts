...
async function cleanupOldDemoAgents(headers: Record<string, string>): Promise<void> {
  try {
    ...
    // Revoke ALL active Nexus Ops demo agents (own type) to ensure room for the new one
    for (const agent of agents) {
      if (agent.name?.includes('Nexus Ops') && agent.name?.includes('(demo-') && agent.lifecycle_state === 'active') {
