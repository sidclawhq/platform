    ...
    <div className={`rounded-lg border ${borderColor} bg-[#111113] p-4`}>
      ...
      {service.alert && (
        <div className="mb-3 rounded border border-[#F59E0B]/20 bg-[#F59E0B]/5 px-3 py-2">
          <span className="text-sm text-[#F59E0B]">{service.alert}</span>
        </div>
      )}
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          <span className="text-base font-medium text-[#E4E4E7]">{service.name}</span>
          <span className={`rounded px-1.5 py-0.5 text-xs font-medium uppercase ${statusColors[service.status]}`}>
            {service.status}
          </span>
        </div>
        <span className="font-mono text-xs text-[#71717A]">{service.region}</span>
      </div>
      ...
      <div className="mt-3 grid grid-cols-4 gap-3 border-t border-[#2A2A2E] pt-3">
        <div>
          <div className="text-xs text-[#71717A]">Error Rate</div>
          <div className={`font-mono text-sm ${service.errorRate > 1 ? 'text-[#EF4444]' : 'text-[#E4E4E7]'}`}>
            {service.errorRate}%
          </div>
        </div>
        <div>
          <div className="text-xs text-[#71717A]">Req/s</div>
          <div className="font-mono text-sm text-[#E4E4E7]">{service.requestsPerSecond.toLocaleString()}</div>
        </div>
        <div>
          <div className="text-xs text-[#71717A]">Instances</div>
          <div className="font-mono text-sm text-[#E4E4E7]">{service.instances}</div>
        </div>
        <div>
          <div className="text-xs text-[#71717A]">Version</div>
          <div className="font-mono text-sm text-[#E4E4E7]">{service.version}</div>
        </div>
      </div>
