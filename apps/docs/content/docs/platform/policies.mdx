...
```bash
curl -X POST https://api.sidclaw.com/api/v1/policies \
...
```bash
curl "https://api.sidclaw.com/api/v1/policies/{id}/versions?limit=10&offset=0" \
  -H "Authorization: Bearer $API_KEY"
```
...
```bash
curl -X PATCH https://api.sidclaw.com/api/v1/policies/{id} \
  -H "Authorization: Bearer $API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "policy_effect": "deny",
...
```bash
curl -X DELETE https://api.sidclaw.com/api/v1/policies/{id} \
  -H "Authorization: Bearer $API_KEY"
```
...
```bash
curl "https://api.sidclaw.com/api/v1/policies?agent_id={agent_id}&effect=approval_required&is_active=true" \
  -H "Authorization: Bearer $API_KEY"
```
...
```bash
curl -X POST https://api.sidclaw.com/api/v1/policies/test \
  -H "Authorization: Bearer $API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "agent_id": "your-agent-id",
