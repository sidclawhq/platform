...
```bash
curl -X POST https://api.sidclaw.com/api/v1/agents/{id}/suspend \
  -H "Authorization: Bearer $API_KEY"
```
...
```bash
curl -X POST https://api.sidclaw.com/api/v1/agents/{id}/revoke \
  -H "Authorization: Bearer $API_KEY"
```
...
```bash
curl -X POST https://api.sidclaw.com/api/v1/agents/{id}/reactivate \
  -H "Authorization: Bearer $API_KEY"
```
...
```bash
curl "https://api.sidclaw.com/api/v1/agents?environment=prod&lifecycle_state=active&limit=20&offset=0" \
  -H "Authorization: Bearer $API_KEY"
```
...
```bash
curl https://api.sidclaw.com/api/v1/agents/{id} \
  -H "Authorization: Bearer $API_KEY"
```
...
```bash
curl -X PATCH https://api.sidclaw.com/api/v1/agents/{id} \
  -H "Authorization: Bearer $API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
