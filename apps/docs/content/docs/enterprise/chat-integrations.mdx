...
description: Get approval notifications with Approve/Deny buttons in Slack, Microsoft Teams, or Telegram.
---
...
| **Microsoft Teams** | Adaptive Card | Links to dashboard | Low |
...
Teams uses incoming webhook connectors. Due to Teams webhook limitations, buttons link to the dashboard rather than performing actions directly in chat.

**1. Create an Incoming Webhook**

In your Teams channel, go to **Connectors > Incoming Webhook** and create a new connector.
...
In the dashboard, go to **Settings > Integrations > Microsoft Teams** and enter the webhook URL.

### What the Message Looks Like

The Adaptive Card includes:

- Agent name and risk level (color-coded)
- The action and target integration (monospace)
- Why the action was flagged
- **Review & Approve** and **Review & Deny** buttons that open the dashboard
...
- **Teams** uses incoming webhooks (outbound only) with no callback verification needed.
...
**Teams cards not appearing?** Verify the incoming webhook URL is correct and the connector is still active.
