...
```python
client.record_outcome(trace_id: str, params: RecordOutcomeParams) -> None
```

`RecordOutcomeParams` is a `TypedDict` with `status` (`"success"` or `"error"`) and optional `metadata` (dict).
...
if decision.decision == "allow":
    try:
        ...
        client.record_outcome(decision.trace_id, {
            "status": "success",
