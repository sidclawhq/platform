...
SidClaw is listed on the [official MCP Registry](https://registry.modelcontextprotocol.io) as `io.github.VladUZH/sidclaw-governance-mcp`. Install it from Claude Desktop, Cursor, or any MCP-compatible client.
...
1. The agent calls a tool through the governance server.
...
The governance proxy supports two transport modes:

| Mode | Use Case | Clients |
|------|----------|---------|
| **stdio** (default) | Local proxy wrapping an upstream MCP server | Claude Desktop, Cursor, OpenClaw |
| **http** | Remote server accessible over HTTPS | Microsoft Copilot Studio, GitHub Copilot, VS Code |

### HTTP Mode (Streamable HTTP)

Start the proxy as an HTTP server for remote MCP clients:
...
All `/mcp` requests require `Authorization: Bearer <api-key>` header.

For detailed setup guides, see:
- [Microsoft Copilot Studio](/docs/integrations/copilot-studio)
- [GitHub Copilot (VS Code)](/docs/integrations/github-copilot)
- [NVIDIA NemoClaw](/docs/integrations/nemoclaw) — Use the MCP proxy inside NemoClaw sandboxes

### stdio Mode (default)

The default mode wraps a local upstream MCP server. See the configuration below.
...
import type { GovernanceMCPServerConfig } from '@sidclaw/sdk/mcp';
...
The CLI binary works the same way regardless of whether you installed via npm or pip:
...
Tool mappings work identically to the TypeScript version. Pass them in the config:

```python
from sidclaw.mcp import GovernanceMCPServer, GovernanceMCPServerConfig, ToolMapping
...
server = GovernanceMCPServer(GovernanceMCPServerConfig(
    ...
    tool_mappings=[
        ...
        ToolMapping(tool_name="list_tables", skip_governance=True),
    ],
