...
Unlike Slack, Teams, and Telegram integrations, email notifications do not support inline approve/deny. Reviewers click through to the dashboard to make their decision.
...
Email notifications are dispatched alongside chat integrations (Slack, Teams, Telegram) — they are not mutually exclusive. All enabled channels fire in parallel.
...
Subsequent approvals are still visible in the dashboard and delivered via chat integrations (Slack, Teams, Telegram) if configured.
...
- Reviewers can still approve/deny via the dashboard, Slack, Teams, or Telegram
...
- If you need higher throughput, consider using Slack, Teams, or Telegram for real-time notifications and email as a fallback
