...
| **Webhook Secret** | A secret string for verifying incoming callbacks — **required** for interactive buttons |
| **Enabled** | Check the box |

All three fields (Bot Token, Chat ID, and Webhook Secret) must be configured for interactive Approve/Deny buttons to work. If any field is missing, button clicks will be rejected with a 403 error (fail-closed). The Webhook Secret is passed as `secret_token` to Telegram's `setWebhook` API and verified via the `X-Telegram-Bot-Api-Secret-Token` header on every incoming callback.

Click **Save**. SidClaw automatically registers the webhook URL (and secret token) with Telegram when you save a new bot token, so there is no manual webhook setup required.
