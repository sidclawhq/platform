...
```bash
Authorization: Bearer ai_example_abc123def456...
```
...
Requests are rate-limited per tenant, categorized by endpoint type. Limits are enforced over a 60-second sliding window. Contact us if you need higher limits.

Unauthenticated requests (without a valid API key or session) are rate-limited to 30 requests per minute per IP address to protect public endpoints from abuse.
