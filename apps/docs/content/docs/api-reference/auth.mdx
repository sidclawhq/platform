...
```bash
curl -H "Authorization: Bearer ai_example_abc123def456..." \
...
```json
{
  "data": {
    ...
    "key": "ai_example_abc123def456...",
    "key_prefix": "ai_example_abc",
...
curl -X POST http://localhost:4000/api/v1/api-keys \
  -H "Authorization: Bearer ai_example_admin_key" \
...
```json
{
  "data": {
    "id": "key-001",
    "name": "Production SDK Key",
    "key": "ai_example_new_key_value...",
    "key_prefix": "ai_example_new",
    "scopes": ["evaluate", "traces:read", "traces:write"]
  }
}
```
