...
describe('Email/password signup', () => {
  ...
  it('API key has default scopes', async () => {
    ...
    const dbKeys = await prisma.apiKey.findMany({ where: { tenant_id: body.data.tenant.id } });
    expect(dbKeys.length).toBe(1);
    // Runtime-only by design — see DEFAULT_SIGNUP_KEY_SCOPES. This key is
    // written into every project generated by create-sidclaw-app, so it must
    // not carry agents:write, policies:write, agents:lifecycle or
    // approvals:write; each of those defeats governance rather than using it.
    expect(dbKeys[0]!.scopes).toEqual([
      'evaluate',
      'traces:read',
      'traces:write',
      'approvals:read',
